...
function Base.write(hdf_file::Union{HDF5.File,HDF5.Group}, path::String, obj::SparseMatFacModel)
    for field in fieldnames(SparseMatFacModel)
        write(hdf_file, string(path, "/", field), getfield(obj, field))
    end
end
...
function readtype(hdf_file, path::String, t::Type{SparseMatFacModel})
...
function model_from_hdf(hdf_file, path::String)
    ...
    return readtype(hdf_file, path, SparseMatFacModel)
